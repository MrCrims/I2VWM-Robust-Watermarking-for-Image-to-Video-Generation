...
class Unet1(nn.Module):
    def __init__(self, params, width=32, ndown=4, nmiddle=0, activ='relu'):
        super().__init__()
        self.secret_len = params.message_length
        self.ndown = ndown
        self.resolution = params.resolution
        self.secret2image = Secret2Image(params.resolution, self.secret_len)
    ...
    def blend(self, encoded, origin):
        encoded_images = self.scaling_i * origin + self.scaling_w * encoded
        encoded_images = self.jnd(origin, encoded_images)
...
class MSResNet(nn.Module):
    def __init__(self, input_dim=3, dim=64, norm='none', activ='lrelu', n_layer=4, gan_type='lsgan', num_scales=3, pad_type='reflect', out_dim=64):
        ...
        if self.gan_type == 'lsgan':
            ...
            self.criterion = lambda real, fake: torch.mean(nn.ReLU()(1 - real)) + torch.mean(nn.ReLU()(1 + fake))
        else: 
    ...
    def compute_loss(self, outputs, choice='real'):
        total_loss = 0.0
        for scale_idx in range(self.num_scales):
            logits = outputs[scale_idx]
        
            label = torch.ones_like(logits)*0.95 if choice=='real' else torch.zeros_like(logits)*0.05
            if self.gan_type == 'lsgan':
                loss = self.criterion(logits, label)
            elif self.gan_type == 'hinge':
                loss = torch.mean(nn.ReLU()(1 - logits))
            else:  
                loss = self.criterion(logits,label)

            scale_loss = loss * self.scale_weights[scale_idx]
            total_loss += scale_loss

        return total_loss
    
    def compute_d_loss(self, real_outputs, fake_outputs):
        d_loss = 0.0
        for scale_idx in range(self.num_scales):
            real_logits = real_outputs[scale_idx]
            fake_logits = fake_outputs[scale_idx]

            if self.gan_type == 'lsgan':
                real_loss = self.criterion(real_logits, torch.ones_like(real_logits)*0.95)
                fake_loss = self.criterion(fake_logits, torch.zeros_like(fake_logits)*0.05)
            elif self.gan_type == 'hinge':
                real_loss = torch.mean(nn.ReLU()(1 - real_logits))
                fake_loss = torch.mean(nn.ReLU()(1 + fake_logits))
            else: 
                real_loss = self.criterion(real_logits, torch.ones_like(real_logits)*0.95)
                fake_loss = self.criterion(fake_logits, torch.zeros_like(fake_logits)*0.05)

            scale_loss = (real_loss + fake_loss) * self.scale_weights[scale_idx]
            d_loss += scale_loss

        return d_loss, None

    def compute_g_loss(self, fake_outputs):
        g_loss = 0.0
        for scale_idx in range(self.num_scales):
            fake_logits = fake_outputs[scale_idx]
            if self.gan_type == 'lsgan':
                g_loss += self.criterion(fake_logits, torch.ones_like(fake_logits)*0.05)
            elif self.gan_type == 'hinge':
...
class ResNetDiscriminator(nn.Module):
    ...
    def compute_loss(self, outputs, choice='real'):
        ...
        labels = torch.full((batch_size, 1), self.cover_label, device=device) if choice=='real' else torch.full((batch_size, 1), self.encoded_label, device=device)
    ...
    def compute_d_loss(self, real_outputs, fake_outputs):
        ...
        d_target_label_cover = torch.full((batch_size, 1), self.cover_label, device=device)
        d_target_label_encoded = torch.full((batch_size, 1), self.encoded_label, device=device)

        d_loss_on_cover = torch.nn.functional.binary_cross_entropy_with_logits(real_outputs, d_target_label_cover.float())
        d_loss_on_encoded = torch.nn.functional.binary_cross_entropy_with_logits(fake_outputs, d_target_label_encoded.float())

        return d_loss_on_cover, d_loss_on_encoded
    
    def compute_g_loss(self, fake_outputs):
        batch_size = fake_outputs.shape[0]
        device = fake_outputs.device
        g_target_label_encoded = torch.full((batch_size, 1), self.cover_label, device=device)

        g_loss_adv = torch.nn.functional.binary_cross_entropy_with_logits(fake_outputs, g_target_label_encoded.float())
