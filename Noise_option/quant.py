...
class RandomizedQuantization(nn.Module):
    def __init__(self, opt ,region_num=8, collapse_to_val = 'inside_random', spacing='random', transforms_like=False, p_random_apply_rand_quant = 1):
        """
        region_num: int;
        """
        super().__init__()
        self.region_num = opt['noise']['Quant']['num']
    ...
    def forward(self, x, encoed=None):
        """
        x: (B, c, H, W) or (C, H, W)
        """
        EPSILON = 1
