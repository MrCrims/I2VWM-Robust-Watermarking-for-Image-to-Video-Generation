...
class JpegBasic(nn.Module):
	...
	def std_quantization(self, image_yuv_dct, scale_factor, round_func):
	...
	def dct(self, image):
		...
		split_num_h = image.shape[2] // 8
		split_num_w = image.shape[3] // 8
		...
		image_dct = torch.cat(torch.cat(image_dct.chunk(split_num_h, 0), 3).chunk(split_num_w, 0), 2)
	...
	def idct(self, image_dct):
		...
		split_num_h = image_dct.shape[2] // 8
		split_num_w = image_dct.shape[3] // 8
		...
		image = torch.cat(torch.cat(image.chunk(split_num_h, 0), 3).chunk(split_num_w, 0), 2)
	...
	def subsampling(self, image, subsample):
		# print(image.shape)
		# h,w = image.shape[2:]
		# image = torch.nn.functional.interpolate(image,size=(((h//8)//2)*16, ((w//8)//2)*16))
		if subsample == 2:
			split_num_h = image.shape[2] // 8
			split_num_w = image.shape[3] // 8
			...
			temp = torch.cat(image_block.chunk(split_num_h, 0), 3)
			image = torch.cat(temp.chunk(split_num_w, 0), 2)
		return image


class Jpeg(JpegBasic):
	def __init__(self, Q, subsample=2, differentiable=True):
		super(Jpeg, self).__init__()

		# quantization table
		self.Q =  random.uniform(Q[0],Q[1]) if isinstance(Q, list) else Q
	...
	def forward(self, image, cover_img=None):
		...
		noised_image = torch.nn.functional.interpolate(noised_image,size=(h,w))
